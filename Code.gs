...
const CONFIG = {
  ...
  HEADERS: [
    ...
    'Articles standards personnalisés', 'Droits - Actif'
  ]
};
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    try {
      results.push({
        ...
        droits: {
          actif: String(row[32] || '') !== 'Non', // absent = Oui (rétrocompat)
          montant: String(row[19] || ''),
          tva: String(row[20] || '')
        },
...
function saveConvention(data) {
  ...
  var rowData = [
    ...
    data.droits.montant,                              // col 20 (index 19)
    data.droits.tva,                                   // col 21 (index 20)
    data.transport.actif ? 'Oui' : 'Non',             // col 22 (index 21)
    data.transport.details || '',                      // col 23 (index 22)
    data.hebergement.actif ? 'Oui' : 'Non',           // col 24 (index 23)
    data.hebergement.details || '',                    // col 25 (index 24)
    data.actionsCulturelles.actif ? 'Oui' : 'Non',   // col 26 (index 25)
    data.actionsCulturelles.details || '',             // col 27 (index 26)
    data.dateSignature,                                // col 28 (index 27)
    isNew ? (data.statut || 'Brouillon') : (data.statut || null), // col 29 (index 28)
    '', // Lien PDF                                    // col 30 (index 29)
    JSON.stringify(data.articlesPerso || []),          // col 31 (index 30)
    JSON.stringify(data.articlesStd || {}),            // col 32 (index 31)
    (data.droits.actif === true || data.droits.actif === 'true') ? 'Oui' : 'Non' // col 33 (index 32)
  ];
...
function generateConventionHtml(data, logos) {
  ...
  html += '</article>';
  // Article 6 — Droits de présentation publique (uniquement si conditions financières actives)
  if (data.droits.actif !== false) {
  n = nextArticle();
  html += '<article class="article no-break">';
